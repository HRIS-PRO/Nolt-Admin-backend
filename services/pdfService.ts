  ...
  generateAndUploadIndemnityPdf: async (
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Reset for body
        doc.fillColor('#000000').font('Helvetica').fontSize(10).lineGap(2);
        doc.moveDown(6); // Move past header
        ...
        doc.font('Helvetica-Bold').fontSize(9).text('Email Address (This email address must be one that previously exists in the Company’s records)').moveDown(0.4);
        doc.font('Helvetica').fontSize(10).text(`Primary Email: ${primaryEmail || '____________________'}`);
        doc.text(`Alternate Email: ${alternateEmail || '____________________'}`).moveDown(1.5);

        // Date and Customer Name
        doc.text(`Date: ${dateString || '____________________'}`);
        doc.text(`Customer Name: ${safeCustomerName}`).moveDown(1.5);
